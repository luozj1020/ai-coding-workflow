...
- monitor-triage: read only the supplied compact local monitor JSON. Never request or inspect raw process listings, full progress logs, full status output, network tails, or source diffs. Return exactly these fields, one per line: decision=continue|inspect|interrupt-candidate|uncertain; confidence=high|medium|low; reason_code=<one-kebab-case-code>; summary=<at most 240 characters compressing only the supplied evidence>; codex_review_required=yes|no; interrupt_authorized=no. Distinguish editing readiness from a durable product write and report product-idle duration/confirmations when present. Prefer continue when useful artifact/progress growth exists and no explicit deviation is present. Time, PID state, or changed-file count alone never proves direction deviation. Use interrupt-candidate only for an explicit local direction-deviation signal or corroborated repeated L3 stale evidence with no growth. Use uncertain for missing or conflicting evidence. This mode is advisory and never kills a process or authorizes interruption.
EOF
