...
def main():
 p=argparse.ArgumentParser();p.add_argument("input");a=p.parse_args();d=json.loads(Path(a.input).read_text(encoding="utf-8"));allowed=d.get("allowed_paths",[]);changed=d.get("changed_files",[])
