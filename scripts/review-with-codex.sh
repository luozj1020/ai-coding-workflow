...
- Check Small Change Fast Path Gate if present or if Claude dispatch was skipped. Verify a fresh pre-card Spark estimate (or recorded deterministic tiny skip), its 1.5x/2.0 calibration, no more than two targeted files, local context, clear solution, high confidence, favorable delegation economics, and preservation of narrow validation evidence or a validation-skip reason. Risk flags must increase review/validation/isolation/approval rigor but must not push work from Codex to Claude. If a human/policy explicitly applied a risk-based owner override, it may bias only toward Codex and must be recorded. Actual edits exceeding the estimate are acceptable when scope, solution, and context remained stable; require a fresh route only after material expansion.
