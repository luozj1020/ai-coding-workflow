...
def main():
 ...
 dispatch_card=card
 if plan["execution"].get("single_pass_builder_checker"):
 ...
 preview={"task_id":plan["task_id"],"lane":plan["lane"],"dispatch_card":str(dispatch_card),"single_pass":plan["execution"].get("single_pass_builder_checker",False),"call_index":len(calls)+1,"execute":a.execute};(out/"dispatch-preview.json").write_text(json.dumps(preview,sort_keys=True,indent=2)+"\n");print(json.dumps(preview,sort_keys=True,indent=2))
