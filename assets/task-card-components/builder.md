...
- Implement only the scoped production change. For aggregation/gate logic, preserve frozen counterexamples and fail closed on missing/contradictory evidence.
- Do not write acceptance tests or run broad validation unless explicitly assigned.
- Continue after a non-blocking `proceed` acknowledgement in the same run; a `split-required` task-shape result remains a control-plane stop.
