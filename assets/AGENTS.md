...
- Use `Small Change Fast Path Gate` before dispatch for tiny low-risk edits. When task size is unclear, call Spark with a short pre-task-card brief before authoring the full task card. Use `execution-cost-estimator` (or `preflight-bundle`) to compare direct Codex editing cost with delegation overhead. Codex fast path is allowed only when the economic recommendation favors it AND the deterministic safety gate passes: <=2 files, local context, low/none validation, high confidence, no risk flags, and upper diff within threshold (`--fast-path-max-diff-lines N` / `CODEX_FAST_PATH_MAX_DIFF_LINES`, default 100, valid 1..200). Generate a full task card only if routing rejects fast path or selects delegation/spec-first. If scope expands or uncertainty appears, stop fast path and return to task-card + Claude dispatch.
