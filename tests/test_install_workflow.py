...
class InstallWorkflowTests(unittest.TestCase):
    ...
    def test_installed_dispatch_has_observability_and_timeout(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('CLAUDE_CODE_WORKTREE_STRATEGY="${CLAUDE_CODE_WORKTREE_STRATEGY:-fresh}"', dispatch)
            self.assertIn('CLAUDE_CODE_LARGE_REPO_MODE="${CLAUDE_CODE_LARGE_REPO_MODE:-0}"', dispatch)
            self.assertIn('CLAUDE_CODE_TASK_CARD_VIEW="${CLAUDE_CODE_TASK_CARD_VIEW:-execution}"', dispatch)
            self.assertIn('CLAUDE_CODE_CHECKER_DISCOVER="${CLAUDE_CODE_CHECKER_DISCOVER:-0}"', dispatch)
    ...
    def test_installed_checker_helper_and_review_prompt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("CLAUDE_CODE_TASK_CARD_VIEW=compact", task_template)
