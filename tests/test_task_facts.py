...
class TaskFactsTests(unittest.TestCase):
 ...
 def test_solution_planner_converges_then_freezes_once(self):
  ...
  opt_in_only={k:v for k,v in base.items() if k!='claude_role'};self.assertEqual(router.route(opt_in_only)['execution']['claude_role'],'solution-planner')
  legacy={k:v for k,v in base.items() if k not in ('claude_role','solution_planner_opt_in')};legacy['allow_claude_planner']=True;self.assertEqual(router.route(legacy)['execution']['claude_role'],'solution-planner')
 ...
 def test_bookend_absorbs_solution_planning_into_owner_convergence(self):
  no={k:'no' for k in RISK_KEYS};facts={'effective_risks':no,'target_files_count':8,'predicted_diff_lines':600,'exact_validation':False,'delegation_value':True,'claude_role':'solution-planner','solution_planner_opt_in':True,'goal_clarity':'medium','implementation_path_clarity':'low','bounded_exploration_scope':True,'durable_structured_output':True,'repository_size':'large','bookend_owned':True}
  decision=router.route(facts);self.assertEqual(decision['execution']['owner'],'claude-builder');self.assertEqual(decision['execution']['claude_role'],'exploratory-builder');self.assertTrue(decision['planning']['solution_planning_internal_to_owner']);self.assertFalse(decision['planning']['solution_contract_required'])
 def test_solution_planner_is_never_automatic_without_explicit_opt_in(self):
  no={k:'no' for k in RISK_KEYS};shape={'effective_risks':no,'target_files_count':8,'predicted_diff_lines':600,'exact_validation':False,'delegation_value':True,'goal_clarity':'high','implementation_path_clarity':'low','bounded_exploration_scope':True,'durable_structured_output':True,'multi_phase_task':True}
  inferred=router.route(shape);self.assertEqual(inferred['execution']['claude_role'],'execution-builder');self.assertEqual(inferred['planning']['strategy'],'codex-contract-freeze-then-claude-convergence');self.assertTrue(inferred['planning']['solution_planner_automatic_candidate_suppressed']);self.assertEqual(inferred['planning']['solution_planner_skip_reason'],'explicit-opt-in-required')
  requested=router.route({**shape,'claude_role':'solution-planner'});self.assertEqual(requested['execution']['claude_role'],'execution-builder');self.assertEqual(requested['planning']['draft_owner'],'codex');self.assertFalse(requested['planning']['solution_planner_opt_in'])
 ...
 def test_precard_spark_skip_requires_complete_deterministic_evidence(self):
  ...
  candidate=router.route({'effective_risks':no,'target_files_count':8,'predicted_diff_lines':500,'exact_validation':False,'claude_role':'solution-planner','solution_planner_opt_in':True,'goal_clarity':'high','implementation_path_clarity':'low','bounded_exploration_scope':True,'durable_structured_output':True,'expected_codex_work_reduction_ratio':.4,'multi_phase_task':True,'spark_route_requested':True})
  self.assertEqual(candidate['precard_estimator']['spark_action'],'estimate')
 def test_registration_and_downstream_field(self):
  installer=(SCRIPTS/'install_workflow.py').read_text();self.assertIn('collect-task-facts.py',installer);self.assertIn('routing-facts-v1.schema.json',installer);schema_value=json.loads((ROOT/'schemas/routing-facts-v1.schema.json').read_text());self.assertEqual(schema_value['properties']['schema_version']['const'],1);self.assertIn('solution_planner_opt_in',schema_value['properties']);self.assertIn('"facts":"collect-task-facts.py"',(SCRIPTS/'aiwf.py').read_text());self.assertNotIn('single_pass_builder_checker',(SCRIPTS/'efficiency-control.py').read_text());self.assertNotIn('single_pass_builder_checker',(SCRIPTS/'dispatch-efficient.py').read_text())
