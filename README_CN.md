...
|---|---|
| Codex 额度稀缺，同时 Claude Code 接入的国产高性价比模型额度充足 | 修改极小、明确或紧急 |
| 功能、迁移、批量修改、测试或长验证能够形成持久交付物 | 只是代码问答、审查或只读调查 |
| 可以接受较长单任务时延，尤其是多个仓库由不同终端同时推进 | 单任务强调低时延或需要紧密交互式调试 |
| 能提供 worktree 隔离和可确定性复核的证据 | Claude 执行、隔离或可靠证据不可用 |
...
flowchart TD
    ...
    C --> CPL[Claude Solution Planner · 一次结构化契约]
    CPL --> CAR[Codex · 一次对抗性规划审查]
    CAR --> CF[本地工具 · 冻结契约 / 非阻塞项进入 Backlog]
    CF --> D
    C --> D[Claude Exploratory/Batch/Execution Builder · 隔离执行]
...
完整控制循环为 **OBSERVE → ROUTE → PLAN → EXECUTE → VERIFY → REVIEW**。
...
智能体运行时不要让 Codex 为每次 heartbeat 醒来。Dispatcher 默认自动启动 `monitor-claude.sh`，本地 supervisor 在 Claude 继续运行时只记录有意义的状态跃迁。机器模式复用 Dispatcher heartbeat 字段，将 diff/status 扫描推迟到决策边界；监控间隔默认跟随 `CLAUDE_CODE_HEARTBEAT_SECONDS`，也可用 `CLAUDE_CODE_BACKGROUND_MONITOR_INTERVAL_SECONDS` 覆盖。稳定的 `continue`/`terminal` 不调用模型；只有 `inspect` 或 `interrupt-candidate` 才异步调用限频的 Spark `monitor-triage`。Spark 只看到紧凑 JSON，不读取原始 `ps`、完整 progress/status、网络尾部或源码 diff。可用 `CLAUDE_CODE_BACKGROUND_MONITOR=0` 关闭 supervisor，用 `CLAUDE_CODE_MONITOR_SPARK=off` 保留纯本地监控，或通过 `CLAUDE_MONITOR_SPARK_MIN_INTERVAL_SECONDS` 调整默认 120 秒限频。Codex只审查仍有疑义或可能中断的结果，任何 helper 都不会自行授权或执行中断。
