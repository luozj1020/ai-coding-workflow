...
小型局部修改可以走 Codex-only fast path，不必派发 Claude，判断依据是校准后的规模、文件数、上下文充分度、方案明确度、置信度和派发经济性。风险标志通常只提高审查、验证、隔离或审批强度，不得把任务从 Codex 推向 Claude；显式风险 owner override 只能倾向 Codex。需要记录为什么没有派发 Claude、触及文件、验证证据，以及什么范围/方案/上下文的实质扩大会触发重新路由。
...
- `execution-cost-estimator`：只读模式，预测任务的 diff 范围/文件数和相对直接/委托工作量。工作量是相对估算，不是 token 计量。包含在 `preflight-bundle` 和 `task-size-classifier` 输出中。每次编写任务卡前都要重新估算，并用 `--routing-event` 标记事件。确定性 owner gate 使用校准后的上界（通常 1.5 倍，编排/测试/跨平台任务 2.0 倍）；风险通常只改变下游审查强度，不得把任务从 Codex 推向 Claude，显式风险 override 只能倾向 Codex。阈值：`--fast-path-max-diff-lines N` / `CODEX_FAST_PATH_MAX_DIFF_LINES`（默认 100，有效范围 1..200）。仅用于派发前决策；永远不会自动编辑源码。
...
`execution-cost-estimator` 模式及其在 `preflight-bundle`/`task-size-classifier` 中的包含支持 `--fast-path-max-diff-lines N` 标志（也可用 `CODEX_FAST_PATH_MAX_DIFF_LINES` 环境变量）来配置 Codex fast-path 资格的校准后 diff 行数上界。默认值为 100，有效范围为 1..200。helper 会先把 Spark 原始上界乘以 1.5 或 2.0，再与该阈值比较；校准后上界超过阈值时，无论经济建议如何，owner gate 都会拒绝 Codex fast path。如果编辑过程中范围、方案和上下文仍稳定，实际行数超过估算本身不是停止条件。
...
智能体运行时不要让 Codex 为每次 heartbeat 醒来。`monitor-claude.sh` 会在本地后台运行同一套分层监控，只把证据、动作/层级、角色状态和终态变化写入 `.worktrees/<task-id>.monitor-events.log`。Codex 应在审查或终态边界只读取一次日志尾部；确实需要语义归纳时，再把这份紧凑日志交给 Spark。
