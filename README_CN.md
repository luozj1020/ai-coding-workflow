...
flowchart TD
    ...
    R -- 直接修改有价值 --> CD[Codex · 有界实现]
    CD --> E
    R -- 委派有价值 --> C[Codex · Context Packet / 任务卡]
    C --> P[Codex · 执行计划与调用预算]
    P --> B[控制面 · Model Call Broker]
    B --> D[Claude Builder · 隔离实现]
    D -. heartbeat / progress / diff .-> MON[本地后台监控 · 只记录 L0–L3 实质变化]
    MON -. 审查边界事件尾部 .-> CR
    D --> Q{已有有效进展且遇到语义阻塞?}
    Q -- 否 --> SP[Spark · 机械后检]
...
完整控制循环为 **OBSERVE → ROUTE → PLAN → DISPATCH → EXECUTE → VERIFY → REVIEW → LEARN**。ROUTE 位于任务卡之前，首先判断委派是否真正减少 Codex 的上下文获取或语义复审；如果仍需完整复审，范围明确的任务由 Codex 直接完成。Claude Builder只承担有经济价值的委派实现。Checker/Test按价值触发：确定性本地证据足以关闭验收时不调用模型；需要编写测试、长时间验证或处理大量证据时才使用 Claude Checker。Spark始终只提供建议，不负责派发、验收或合并；最终由人工合并。

这属于职责专业化，而不是全面缩小 Claude 权限。机械批量修改、独立辅助单元、测试编写、长时间验证和大证据处理仍优先交给 Claude；当 Codex 已掌握精确上下文，且委派只会重复一次完整语义复审时，尤其在大型仓库中，才由 Codex 承担有界的核心语义实现。风险只提高隔离与审查强度，并且最多让 owner 向 Codex 倾斜，不能把任务推向 Claude。
...
也可以使用 `CODEX_SPARK_HOST_AUTHORITY=1` 明确授权，并用 `CODEX_SPARK_HOST_RETRY_TIMEOUT` 设置正数超时。`spark-dispatch.json` 会分别记录初始沙箱尝试、交接判断、宿主重试、超时和最终状态。重试超时会终止隔离的重试进程树，随后继续 Claude，不进行第二次重试，也不回退到强模型。没有明确宿主权限时，调度器只记录 `needs_host_execution` 并安全继续。仅在仍受限的沙箱内取消变量不能恢复网络，也不构成宿主权限。需要有意保留标记时使用 `--execution-env sandbox`。持久化报告会记录请求和解析后的执行环境，紧凑失败诊断会保留经过脱敏的 stderr 头尾。
...
- `execution-cost-estimator`：只读模式，预测任务的 diff 范围/文件数、`task_role=core-semantic|auxiliary|mixed|unknown` 和相对直接/委托工作量。工作量是相对估算，不是 token 计量。包含在 `preflight-bundle` 和 `task-size-classifier` 输出中。每次编写任务卡前都要重新估算，并用 `--routing-event` 标记事件。确定性 owner gate 同时使用仓库规模与校准后的上界（通常 1.5 倍，编排/测试/跨平台任务 2.0 倍）；风险通常只改变下游审查强度，不得把任务从 Codex 推向 Claude，显式风险 override 只能倾向 Codex。可用 `--repository-scale small|medium|large|giant` 覆盖自动分类，显式行数阈值仍可用于策略调优。仅用于派发前决策；永远不会自动编辑源码。
...
智能体运行时不要让 Codex 为每次 heartbeat 醒来。`monitor-claude.sh start` 在本地运行分层 watcher，只记录有意义的状态跃迁。到审查边界时，`monitor-claude.sh decision` 先用确定性规则把进程状态、计时、产物增长、进度字段、错误类别和受限数量的变更路径压缩成小型判定包。稳定的 `continue`/`terminal` 不调用模型；只有 `inspect` 或 `interrupt-candidate` 才可能调用 Spark `monitor-triage`。Spark 只看到紧凑 JSON，不读取原始 `ps`、完整 progress/status、网络尾部或源码 diff；Codex 只审查仍有疑义或可能中断的结果。任何 helper 都不会自行授权或执行中断。
...
# 智能体运行推荐：本地后台监控，不让 Codex 轮询 heartbeat
