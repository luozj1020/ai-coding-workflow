...
- A fixed minimal interaction probe via `ai/claude-healthcheck.py --interaction-route auto` is an authorized read-only diagnostic and requires no human review. It stops after the current route succeeds and tries the alternate only after failure. Its recommendation may inform `CLAUDE_CODE_PROXY_MODE`, but it never changes configuration automatically and is not implementation or acceptance evidence.
