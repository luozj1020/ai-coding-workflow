...
- Validation should follow the task card first. Prefer exact checker commands with `ai/check-worktree.sh --no-discover --command 'label=command'`; broad checker discovery is optional and should be enabled only when requested. If Claude cannot run Python/Node/test commands due to approval or sandbox policy, record the blocked command and let Codex/human rerun it instead of treating the implementation as failed.
