...
2. ROUTE: before every full task-card authoring event, use a 20–40 line Spark `execution-cost-estimator` or `preflight-bundle` brief for every non-obvious-tiny task; this includes initial, revision, narrow, retry, re-dispatch, split-child, and next-phase cards, identified with `--routing-event initial|revision|narrow|retry|next-phase`. Deterministic Express/tiny work may skip only with `skip.sized_tiny_fastpath`. Choose Codex fast path, task-card/Claude dispatch, spec-first, or clarification and record the invoke/skip reason. Do not reuse an earlier card's owner decision.
...
- Classify failed rounds with `ai/classify-claude-attempt.py` before retry/takeover accounting. A DNS/connection/TLS/timeout/API transport failure before acknowledgement, diff, report, or meaningful progress is `transient-transport`: retry the same worktree at most once and do not count it toward the no-progress takeover threshold. Useful evidence produced before a transport failure remains reviewable. `acknowledgement only`, a clean exit with no progress, and confirmed direction deviation count; approval/sandbox blockers do not. Advisory endpoint probes never override a successful Claude interaction.
- A fixed minimal interaction probe via `ai/claude-healthcheck.py --interaction-route auto --timeout 60` is an authorized read-only diagnostic and requires no human review. Its default and dispatcher zero-output prompt is exactly `你好`, equivalent to `claude -p '你好'`. Before attributing a zero-usable-output round to model no-progress, run it once in the same resolved route and preserve the machine-readable interaction-health artifact. `unavailable-in-current-environment` and `inconclusive-restricted-environment` do not count toward takeover. A failed probe under `CODEX_SANDBOX_NETWORK_DISABLED=1` is inconclusive, not proof that Claude or the provider is unavailable; a successful user-terminal interaction is authoritative. The probe never changes configuration or route preference and is not implementation or acceptance evidence. Successful dispatches persist the proven `direct`/`inherit` route through `ai/claude-route-preference.py`; explicit `CLAUDE_CODE_PROXY_MODE` remains authoritative.
- When Claude has useful on-plan diff/report/progress and one semantic blocker, `aiwf advisor-continuation` may prepare a bounded one-call advisor packet and, after an explicit response is supplied, a same-worktree continuation card. Zero progress, transport, approval, direction deviation, repeated advisor use, out-of-scope evidence, and oversized evidence fail closed. The helper invokes no model and dispatches nothing; advisor guidance remains reviewable input, not acceptance or merge authority.
- Advisor execution requires a non-empty request/evidence binding and an explicit call cap of one. The Broker enforces `request_id` idempotency across roles, records interrupts as cancelled, and accounts fixed interaction probes as non-budget `diagnostic_call` records. Continuation audits aggregate same-worktree success and conservative re-exploration evidence; missing token/time savings remain unavailable.
- Process visibility may also differ across sandbox boundaries. `status-claude.sh` and `watch-claude.sh` must report `visibility-unknown` / `overall_running=unknown` when PID lookup fails inside a restricted sandbox but the dispatch progress has started and has no terminal marker. In that state use `CHECK_OUTSIDE_SANDBOX_DO_NOT_REDISPATCH`, re-run monitoring in the dispatch environment, and never start a second Builder in the same worktree. `CLAUDE_CODE_PROCESS_VISIBILITY=normal|restricted|auto` provides an explicit override for diagnostics.
- Builder mode defaults to `auto` and resolves to `execution-only` only when the task card explicitly marks both Context sufficient and Execution-only eligible; otherwise it remains `standard`. Validate the packet with `ai/validate-claude-context.py`. Prefer one responsibility, 1–5 exact target files, named symbols, one source-of-truth example, forbidden paths, measurable acceptance, and narrow validation. The execution-only first-progress default is 60 seconds; base timeout remains 600 seconds with one 300-second active-progress extension.
...
- For small local edits, Codex should fill or mentally evaluate `Small Change Fast Path Gate` before dispatch. Use Spark `execution-cost-estimator` (or the cost fields in `task-size-classifier`/`preflight-bundle`) to compare direct Codex editing cost with Claude delegation overhead. Multiply Spark's raw upper line estimate by 1.5 normally and 2.0 for tests/fixtures, shell/process orchestration, and cross-platform work. Codex fast path is allowed when the economic recommendation and deterministic owner gate favor it: calibrated upper diff within threshold (`--fast-path-max-diff-lines N` / `CODEX_FAST_PATH_MAX_DIFF_LINES`, default 100, valid 1..200), <=2 files, local context, high confidence, and complete economics. Risk flags and validation complexity increase review, validation, isolation, or approval rigor but do not normally choose Codex versus Claude. Risk must never push a task from Codex to Claude; if a human or policy explicitly applies a risk-based owner override, high risk may bias only toward Codex. Actual edits may exceed the estimate while scope, solution, and context remain stable. Record why Claude was not dispatched, files touched, validation evidence, risk-driven rigor, and the material expansion that would require a fresh route.
- The early routing call must not require a full task card. Use `--brief`, `--brief-file`, or `--stdin-brief`; include only goal, bounded observations, likely scope, known risks, and validation shape. Generate the full task card only after Spark recommends delegation/spec-first or the deterministic owner gate rejects Codex fast path. Run a fresh estimate before every revised, narrowed, retried, re-dispatched, split-child, or next-phase card instead of reusing a prior owner decision.
...
bash ai/run-codex-spark.sh \
  --brief "Goal: fix one parser branch. Evidence: likely one file. Risks: none known. Validation: one focused unit test." \
  --mode execution-cost-estimator \
  --routing-event initial \
  --result-mode direct
```

### Controlled-Builder Permission Mode

A new explicit `controlled-builder` mode provides narrow, auditable source-write permission for Spark. Unlike `micro-builder` which creates its own worktree, `controlled-builder` enforces strict boundaries on what paths and how much can change:

- Source-writing modes (`controlled-builder`, `micro-builder`) force full artifacts and an isolated worktree.
- The task card must specify 1–3 exact `--allow-write` paths, repeated for each allowed path, with a matching `Controlled-builder allowed paths` row.
...
When using `controlled-builder`, the task card must include:

| Field | Value |
|-------|-------|
| Result mode | `full` (forced for source-writing modes) |
| Controlled-builder authorized? | yes |
| Controlled-builder allowed paths | exact 1–3 paths, each `--allow-write` repeated |
| Max files | 3 |
| Max diff lines | <=200 |
...
- For experimental wall-clock reduction, Codex may fill `Parallel Execution Gate` and run `ai/run-parallel-loop.sh`. Parallel remains opt-in; ordinary serial tasks incur no extra model call. The workflow is: (1) local zero-token classifier `python scripts/assess-parallel-opportunity.py --json ...` identifies candidates; (2) only for candidates, one bounded Spark `parallel-planner` call produces a reviewed DAG plan; (3) Codex/human reviews and saves task cards and plan; (4) deterministic validation checks base commit agreement (must match current `HEAD`), write scope overlap, owned contract overlap, and validation ownership before any dispatch; (5) explicit max concurrency 2; (6) serial review and merge. Two compatible paths exist: (a) flat independent cards with positional arguments, and (b) a reviewed DAG plan with `bash ai/run-parallel-loop.sh --plan ai/plans/.../parallel-plan.json`. The plan must be strict schema-v1 JSON produced by Spark `parallel-planner` mode or hand-written, then reviewed and saved by Codex/human before dispatch. Schema fields: `schema_version` (must be `1`), `group_id`, `max_concurrency`, `failure_policy` (currently `skip-dependents`), and `tasks` containing `id`, `task_card`, and `depends_on` per task. Every task card must declare a non-placeholder Base commit; all cards must agree; mismatch with current `HEAD` stops before dispatch. The scheduler starts only dependency-ready tasks up to the concurrency cap; a failed prerequisite skips all transitive dependents while unrelated branches continue. Serial fallback uses a stable topological order. `--allow-overlap` is an explicit manual-reconcile escape hatch that does not bypass contract, base commit, or validation checks. All cards still require scope-gate and overlap checks; review and merge remain serial.
