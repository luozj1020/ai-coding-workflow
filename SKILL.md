...
- A fixed minimal interaction probe via `ai/claude-healthcheck.py --interaction-route auto --timeout 30` is an authorized read-only diagnostic and requires no human review. It stops after the current route succeeds and tries the alternate only after failure. Run it in the same network/sandbox context as dispatch: a failed probe under `CODEX_SANDBOX_NETWORK_DISABLED=1` is `inconclusive-restricted-environment`, not proof that Claude or the provider is unavailable. A successful `claude -p` interaction in the user's terminal is authoritative over a failed restricted probe. When terminal/inherit succeeds but dispatcher `direct` fails before interaction, retry the same worktree once with `CLAUDE_CODE_PROXY_MODE=inherit`; classify the first attempt as transient transport and do not count it toward takeover. The health probe itself never changes configuration and is not implementation or acceptance evidence. Successful dispatches persist the proven `direct`/`inherit` route through `ai/claude-route-preference.py`; later unset dispatches reuse it, while an explicit `CLAUDE_CODE_PROXY_MODE` remains authoritative and a successful alternate route replaces the prior preference. Inspect with `python ai/claude-route-preference.py show`; override the state path with `AIWF_CLAUDE_ROUTE_STATE` when needed. Persistence is advisory and stores no proxy values or credentials.
